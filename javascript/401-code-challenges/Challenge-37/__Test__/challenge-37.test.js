...
const businessTrip = require("../business.js");
...
describe("Can use graph", () => {
  ...
  it("Returns True and price, if trip is possible.", () => {
    expect(businessTrip(graph, firstTrip)).toEqual("True, $150");
    expect(businessTrip(graph, secondTrip)).toEqual("True, $115");
  });
  it('Returns False and price, if trip is not possible.', () => {
    expect(businessTrip(graph, thirdTrip)).toEqual('False, $0')
    expect(businessTrip(graph, fourthTrip)).toEqual('False, $0')
  })
  it('Returns False and price, if trip has only 1 city', () => {
    let newTrip1 = ['Pandora']
    let newTrip2 = ['Arendelle']

    expect(businessTrip(graph, newTrip1)).toEqual('False, $0')
    expect(businessTrip(graph, newTrip2)).toEqual('False, $0')
  })
});