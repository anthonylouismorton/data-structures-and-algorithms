...
const repeat = require('../Hashmaprepeat.js');

describe("Can successfully implement a HashTable", () => {

  it('Find the first word repeated in a string', () => {
    ...
    expect(repeat(string1)).toEqual('a')
    expect(repeat(string2)).toEqual('it')
    expect(repeat(string3)).toEqual('was')
    
  });

  it('does not return a word if there are no repeating words', () => {
    const string1 = "there are no repeats here my friend"

    expect(repeat(string1)).toEqual("there are no repeating words")
