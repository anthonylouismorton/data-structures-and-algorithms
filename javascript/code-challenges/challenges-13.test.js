...
const firstLetters = (arr) => arr.map(x => x.split('')[0]);
...
const findHappiness = (arr) => arr.filter(x => x.includes(':)') ? arr : console.log('') );
...
const standardizePhoneNumbers = (arr) => arr.map((x) => x.replace(/[^\d]/g,''));
...
const onlyOddChars = (str) => {
  const newArr = [];
  str.split('').reduce((a,b,i) => i % 2 === 1 ? newArr.push(b) : console.log('e'),[]);
  return newArr.join('');
};
...
const allHappy = (arr) => {
  let includes = true;
  arr.forEach(x => {
    !x.includes(':)') ? includes = false : console.log('');
 });
 return includes;
};
