...
const screenForNames = (arr) => {
  var word = (/([MD]\w+\.\s\w+)/g);
  return arr.filter(str => word.test(str));
}
...
const toTitleCase = (arr) => arr.map(x => x[0].toUpperCase() + x.slice(1))
...
let biggerThanLuke = (arr) => arr.filter(x => x.mass > 77).map(x => x.name).join(' - ')
...
const detectTicTacToeWin = (board) => {
  for(let i = 0; i < board.length; i++){
    for(let j = 0; j < board[i].length; j++){
      if(([0][0] === [0][1] === [0][2] && [i][j] !== (''))|| ([1][0] === [1][1] === [1][2] && [i][j] !== ('')) ||( [2][0] === [2][1] === [2][2] && [i][j] !== ('')) ||( [0][0] === [1][0] === [2][0] && [i][j] !== (''))||( [0][1] === [1][1] === [2][1] && [i][j] !== (''))||( [0][2] === [1][2] === [2][2] && [i][j] !== ('')) || ( [0][0] === [1][1] === [2][2] && [i][j] !== (''))||( [0][2] === [1][1] === [2][0] && [i][j] !== (''))){
        return true;
      }
      else{
        return false;
