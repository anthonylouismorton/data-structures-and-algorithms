...
const evenOdd = (arr) => {
  // Solution code here...
  const newArr = arr.map(x => isNaN(x) ? 'N/A': (x % 2 === 0) ? 'even' : 'odd');
  return newArr;
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  var newArr = arr.map(x => x.ability.name)
  return newArr;
};
...
const extractStats = (arr) => {
  ...
  // });
  var newArr = arr.map(x => {return {name: x.stat.name, total: x.effort + x.baseStat}})
  return newArr;
};
/* ------------------------------------------------------------------------------------------------
TESTS
