...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((bank, value) => {return bank < value ? bank = value: bank;},0);
...
const updateNumbers = (obj) => Object.entries(obj).map(entry => {return `${entry[0]}: ${entry[1]}`})
...
const hasChildrenValues = (arr, character) => {
    ...
    if( person.name === character && person.children){
      ...
      console.log(Object.values(person.children).length)
