...
const toLastNames = people => {
  // Solution code here...
  var newArr = people.map(x => x.firstName + ' ' + x.lastName)
  return newArr;
};
...
const addValues = (arr) => arr.reduce((number, value) => {return number + value;}, 0);
...
const addPurchases = (arr) => {
  // Solution code here...
  var total = arr.reduce(function(number, value){
    number = number + value.purchasePrice;
    return number;
  },0);
  return total;
};
...
// arr.reduce((number, person) => number.concat(person.name), []); 
{
  var name = arr.reduce((number, person) =>{
     return number.concat(person.name);
...
const reversedString = (str) => { return str.split('').reduce((first, last) => (last + first), '');};
...
return arr.reduce((accumulator, parent) => (parent.children ? accumulator + parent.children.length : accumulator ),0)}
...
const countPrimeNumbers = (arr) => arr.reduce((bank, number) => {return isPrime(number) ? bank + 1 : bank},0);
...
const extractStat = (statName, arr) => arr.reduce((bank, str) => {return str.stat.name === statName ? bank = str : bank},null)
  // Solution code here...
...
const extractChildren = (arr) => { 
  // Solution code here...
  var newArr = arr.filter(person => person.name.includes('a'))
    .reduce((bank, parent) => {return parent.children ? bank.concat(parent.children) : bank;},[]);
    return newArr;
};
